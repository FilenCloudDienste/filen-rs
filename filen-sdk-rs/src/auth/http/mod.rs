...
use tower::{
	ServiceBuilder, ServiceExt, limit::GlobalConcurrencyLimitLayer, retry::budget::TpsBudget,
};
...
use crate::{
	...
	auth::{
		...
		http::{
			auth::AuthLayer,
			bandwidth_limit::{
				DownloadBandwidthLimiterLayer, new_download_bandwidth_limiter,
...
pub(crate) struct SharedClientState {
	...
	upload_limiter: Option<Arc<bandwidth_limit::BandwidthLimiter>>,
	...
	pub(crate) fn new(config: ClientConfig) -> Result<Self, Error> {
		Ok(Self {
			...
			upload_limiter: config
				...
				.transpose()?,
			download_limiter: bandwidth_limit::DownloadBandwidthLimiterLayer::new(
				config
...
impl UnauthClient {
		...
		Req: Serialize + Debug,
	{
		ServiceBuilder::new()
			...
			.layer(download_body::full::DownloadLayer) // required to download full response body to bytes
			.layer(bandwidth_limit::UploadBandwidthLimiterLayer::new(
...
impl AuthClient {
		...
		Req: Serialize + Debug,
	{
		...
		ServiceBuilder::new()
			...
			.layer(self.unauthed.state.download_limiter.clone())
