...
//! Based on https://github.com/tower-rs/tower/blob/c1e28af2a9af852f0261883531bbc182121a35fd/tower/src/retry/budget/tps_budget.rs
...
impl TpsBudget {
	...
	fn expire(&self) {
		let mut gen_guard = self.generation.lock().expect("generation lock");

		let now = Instant::now();
		let diff = now.saturating_duration_since(gen_guard.time);
		if diff < self.window {
			// not expired yet
			return;
		}
		...
		self.slots[gen_guard.index].store(to_commit, Ordering::SeqCst);

		let mut diff = diff;
		let mut idx = (gen_guard.index + 1) % self.slots.len();
