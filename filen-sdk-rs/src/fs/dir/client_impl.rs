...
use crate::runtime::do_cpu_intensive;
use crate::{
	api,
	auth::Client,
...
impl Client {
	...
	pub async fn list_dir(
		...
		let files_fut = do_cpu_intensive(|| {
			response
				.files
				.into_par_iter()
				.map(|f| {
		...
		let dirs = dirs_fut.await;
		let files = files_fut.await;

		// let (dirs, files) = futures::join!(dirs_fut, files_fut);
