...
impl Client {
	...
	pub async fn update_file_metadata(
		...
		let name = crypter.encrypt_meta(&temp_meta.name).await;
		let metadata = async {
			let meta_json = serde_json::to_string(&temp_meta)?;
			Ok::<_, Error>(crypter.encrypt_meta(&meta_json).await)
		}
		.await;

		// let (name, metadata) = futures::join!(crypter.encrypt_meta(&temp_meta.name), async {
		// 	let meta_json = serde_json::to_string(&temp_meta)?;
		// 	Ok::<_, Error>(crypter.encrypt_meta(&meta_json).await)
		// });
