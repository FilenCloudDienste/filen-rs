	...
	FileExtra: Clone + std::fmt::Debug + 'static,
{
	...
	pub(super) fn finalize<C>(
		mut self,
	) -> Result<(super::FSTree<DirExtra, FileExtra>, FSStats, Vec<WalkError>), Error>
	where
	...
	fn adjust_stack_until_depth<C>(
		&mut self,
		target_depth: usize,
	) -> (Result<Option<DirChildrenInfo>, Error>, Vec<WalkError>)
	where
		C: CompareStrategy<DirExtra, FileExtra>,
	{
	...
	FileExtra: std::fmt::Debug,
{
	...
	for (name, (first, extra)) in by_name.into_iter() {
		if !extra.is_empty() {
			...
			errors.push(WalkError::DuplicateName(duplicate_path));
		}
