...
use crate::error::{ConversionError, TransparentError};

#[derive(Clone, PartialEq, Eq, Hash)]
#[rkyv_self(no_check_bytes)]
pub struct SizedStr<N: ArrayLength>(pub(super) GenericArray<u8, N>);

unsafe impl<N: ArrayLength, C: Fallible + ?Sized> CheckBytes<C> for SizedStr<N>
where
	C::Error: Source,
{
	unsafe fn check_bytes(value: *const Self, context: &mut C) -> Result<(), C::Error> {
		// first run GenericArray's check_bytes to verify the length
		unsafe { GenericArray::<u8, N>::check_bytes(value.cast(), context)? };
		let bytes = unsafe { &*(value as *const GenericArray<u8, N>) };

		// then verify that the bytes are valid UTF-8
		std::str::from_utf8(bytes.as_slice())
			.map_err(TransparentError::new)
			.into_error()?;
		Ok(())
	}
}

impl<N: ArrayLength> Copy for SizedStr<N> where N::ArrayType<u8>: Copy {}
	...
	N: ArrayLength,
{
	...
	pub(crate) unsafe fn from_bytes_unchecked(bytes: GenericArray<u8, N>) -> Self {
		Self(bytes)
	}
...
impl<N: ArrayLength> Deref for SizedStr<N> {
	type Target = str;
...
impl<N: ArrayLength> std::fmt::Debug for SizedStr<N> {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		write!(f, "StackSizedString<{}>(\"{}\")", N::USIZE, self)
	}
}

impl<N: ArrayLength> std::fmt::Display for SizedStr<N> {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		self.deref().fmt(f)
	}
}

impl<N: ArrayLength> TryFrom<&str> for &SizedStr<N> {
	...
	fn try_from(value: &str) -> Result<Self, Self::Error> {
		...
		// SAFETY: We just verified that the bytes fit into the array, and the input is valid UTF-8, so this is safe.
		Ok(unsafe { &*(array as *const GenericArray<u8, N> as *const SizedStr<N>) })
	}
}

impl<N: ArrayLength> TryFrom<String> for Box<SizedStr<N>> {
	type Error = ConversionError;
...
impl<N: ArrayLength> TryFrom<Box<str>> for Box<SizedStr<N>> {
	type Error = ConversionError;

	fn try_from(value: Box<str>) -> Result<Self, Self::Error> {
		let len = value.len();
		let bytes = value.into_boxed_bytes();
		let array = GenericArray::<u8, N>::try_from_boxed_slice(bytes)
			.map_err(|_| ConversionError::InvalidLength(len, N::USIZE))?;

		// SAFETY: We just verified that the bytes fit into the array, and the input is valid UTF-8, so this is safe.
		Ok(unsafe { Box::from_raw(Box::into_raw(array) as *mut SizedStr<N>) })
	}
}

impl<N: ArrayLength> Serialize for SizedStr<N> {
...
impl<'de, N: ArrayLength> Deserialize<'de> for SizedStr<N>
where
	N::ArrayType<u8>: Copy,
{
...
mod tests {
	...
	use typenum::{U0, U1, U2, U3, U4, U5, U8, U16};
	...
	fn try_from_exact_length_succeeds() {
		let s = <&SizedStr<U5>>::try_from("hello").expect("len matches N");
		assert_eq!(&**s, "hello");
	}

	#[test]
	fn try_from_too_short_fails_with_invalid_length() {
		let err = <&SizedStr<U5>>::try_from("hi").unwrap_err();
		assert!(
	...
	fn try_from_too_long_fails_with_invalid_length() {
		let err = <&SizedStr<U3>>::try_from("hello").unwrap_err();
		assert!(
	...
	fn try_from_empty_string_into_zero_length_succeeds() {
		let s = <&SizedStr<U0>>::try_from("").expect("zero length should accept empty");
		assert_eq!(&**s, "");
		assert_eq!(s.len(), 0);
	}
	...
	fn try_from_multibyte_utf8_byte_exact_succeeds() {
		...
		let s = <&SizedStr<U2>>::try_from("é").unwrap();
		assert_eq!(&**s, "é");

		// "🦀" = 4 bytes → needs N = 4
		let crab = <&SizedStr<U4>>::try_from("🦀").unwrap();
		assert_eq!(&**crab, "🦀");
	}
	...
	fn try_from_multibyte_utf8_wrong_byte_count_fails() {
		...
		let err = <&SizedStr<U1>>::try_from("é").unwrap_err();
		assert!(matches!(err, ConversionError::InvalidLength(2, 1)));

		// 4-byte char does not fit N = 3.
		let err = <&SizedStr<U3>>::try_from("🦀").unwrap_err();
	...
	fn deref_yields_original_str() {
		let s = <&SizedStr<U5>>::try_from("world").unwrap();
		let as_str: &str = s;
	...
	fn deref_enables_str_methods() {
		let s = <&SizedStr<U5>>::try_from("HELLO").unwrap();
	...
	fn deref_preserves_multibyte_chars() {
		let s = <&SizedStr<U4>>::try_from("🦀").unwrap();
	...
	fn display_matches_inner_str() {
		let s = <&SizedStr<U3>>::try_from("abc").unwrap();
	...
	fn debug_for_zero_length() {
		let s = <&SizedStr<U0>>::try_from("").unwrap();
	...
	fn clone_produces_equal_value() {
		let a = <&SizedStr<U5>>::try_from("hello").unwrap();
	...
	fn equal_values_are_eq() {
		let a = <&SizedStr<U5>>::try_from("hello").unwrap();
		let b = <&SizedStr<U5>>::try_from("hello").unwrap();
		assert_eq!(a, b);
	}
	...
	fn different_values_are_not_eq() {
		let a = <&SizedStr<U5>>::try_from("hello").unwrap();
		let b = <&SizedStr<U5>>::try_from("world").unwrap();
		assert_ne!(a, b);
	}

	#[test]
	fn equal_values_hash_equally() {
		let a = <&SizedStr<U8>>::try_from("abcdefgh").unwrap();
		let b = <&SizedStr<U8>>::try_from("abcdefgh").unwrap();
	...
	fn different_values_hash_differently() {
		...
		let a = <&SizedStr<U8>>::try_from("abcdefgh").unwrap();
		let b = <&SizedStr<U8>>::try_from("12345678").unwrap();
	...
	fn deserialize_from_json_exact_length() {
		let s: SizedStr<U5> = serde_json::from_str("\"hello\"").unwrap();
		assert_eq!(&*s, "hello");
	}

	#[test]
	fn deserialize_zero_length() {
		let s: SizedStr<U0> = serde_json::from_str("\"\"").unwrap();
		assert_eq!(&*s, "");
	}

	#[test]
	fn deserialize_wrong_length_errors() {
		let res = serde_json::from_str::<SizedStr<U5>>("\"hi\"");
		assert!(res.is_err());
	...
	fn deserialize_too_long_errors() {
		let res = serde_json::from_str::<SizedStr<U3>>("\"hello\"");
		assert!(res.is_err());
	}

	#[test]
	fn deserialize_non_string_errors() {
		let res = serde_json::from_str::<SizedStr<U5>>("12345");
		assert!(res.is_err());
	}
	...
	fn serde_round_trip_ascii() {
		let original = <&SizedStr<U16>>::try_from("abcdefghijklmnop").unwrap();
		let json = serde_json::to_string(&original).unwrap();
		let decoded: SizedStr<U16> = serde_json::from_str(&json).unwrap();
	...
	fn serde_round_trip_multibyte() {
		...
		let original = <&SizedStr<U4>>::try_from("🦀").unwrap();
		let json = serde_json::to_string(&original).unwrap();
		let decoded: SizedStr<U4> = serde_json::from_str(&json).unwrap();
		assert_eq!(*original, decoded);
	}
	...
	fn rkyv_round_trip_ascii() {
		let original = <&SizedStr<U5>>::try_from("hello").unwrap();
		let bytes = rkyv::to_bytes::<rkyv::rancor::Error>(original).unwrap();
		let decoded = rkyv::from_bytes::<SizedStr<U5>, rkyv::rancor::Error>(&bytes).unwrap();
		assert_eq!(&decoded, original);
	}

	#[test]
	fn rkyv_round_trip_zero_length() {
		let original = <&SizedStr<U0>>::try_from("").unwrap();
		let bytes = rkyv::to_bytes::<rkyv::rancor::Error>(original).unwrap();
		let decoded = rkyv::from_bytes::<SizedStr<U0>, rkyv::rancor::Error>(&bytes).unwrap();
		assert_eq!(&decoded, original);
	}

	#[test]
	fn rkyv_round_trip_multibyte() {
		let original = <&SizedStr<U4>>::try_from("🦀").unwrap();
		let bytes = rkyv::to_bytes::<rkyv::rancor::Error>(original).unwrap();
		let decoded = rkyv::from_bytes::<SizedStr<U4>, rkyv::rancor::Error>(&bytes).unwrap();
		assert_eq!(&decoded, original);
	}

	#[test]
	fn rkyv_rejects_invalid_utf8() {
		// The archived form of `SizedStr<U5>` is the bare 5 UTF-8 bytes (no header).
		// `0xC3` starts a 2-byte sequence but `0x28` is not a valid continuation byte.
		let invalid: [u8; 5] = [0x00, 0x00, 0xC3, 0x28, 0x00];
		let res = rkyv::from_bytes::<SizedStr<U5>, rkyv::rancor::BoxedError>(&invalid);
